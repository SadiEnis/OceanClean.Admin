...
import type { EconomyItemTimeseriesPoint } from '@/features/economy/types'
import { chartSeries } from '@/lib/chart-config'

type EconomyItemTimeseriesChartProps = {
...
function getUniqueItems(points: EconomyItemTimeseriesPoint[]) {
...
export function EconomyItemTimeseriesChart({
                                               points,
                                               valueKey,
                                           }: EconomyItemTimeseriesChartProps) {
    if (points.length === 0) {
        return (
                ...
                <LineChart data={chartRows}>
                    ...
                    {items.map((item, index) => (
                        <Line
                            ...
                            name={item.itemName}
                            stroke={
                                itemColorPalette[index % itemColorPalette.length] ??
                                fallbackColor
                            }
                            strokeWidth={2}
                            dot={false}
